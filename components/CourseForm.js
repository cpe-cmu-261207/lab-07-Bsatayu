    ...
    <>
       <form onSubmit ={
        ...
        <select onChange = { e => 
          ...
          <td>
          ID :
          <input className = "bg-blue-100 rounded-3xl p-1  hover:bg-green-200" type="text" onChange = { e => 
           // setInputData({...inputData,name: e.currentTarget.value}) 
           dispatch({type : 'setname',value : e.currentTarget.value})
          }/>
          </td>
          <td><button className = "bg-red-500 rounded-3xl p-1  hover:bg-green-200" type="submit"> + </button></td> 
       </form>
    </>
  );
};
